...
def run(pnj):
    # initialize input
    rev_bias = list(numpy.arange(0, pnj.stop, 0.5))
    er = pnj.er
    ...
    while i != len(rev_bias):
        # print(rev_bias[i])
        vbarrier.append(0.026 * numpy.log(Na * Nd / (Ni ** 2)) + numpy.abs(rev_bias[i]))
        ...
        vbarrier[i] = 0.026 * numpy.log(Na * Nd / (Ni ** 2)) + numpy.abs(rev_bias[i])
    ...
    first_indexes = numpy.array([first_n_index, first_p_index])
    sides = numpy.column_stack((nside, pside))

    return rev_bias, layer_array, widths, first_indexes, n_indexes, p_indexes, sides